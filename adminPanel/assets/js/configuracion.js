(function () {
    ...
    const configFields = {
        backendPath: document.getElementById('configBackendPath'),
        dbHost: document.getElementById('configDbHost'),
        dbName: document.getElementById('configDbName'),
        dbUser: document.getElementById('configDbUser'),
        dbPort: document.getElementById('configDbPort'),
        whatsappStore: document.getElementById('configWhatsappStore'),
    };
    ...
    async function loadConfigInfo() {
        try {
            const response = await fetch('backend/configuracion.php?action=info');
            if (!response.ok) {
                throw new Error('No se pudo cargar la información de conexión.');
            }
            ...
            const { db, backendPath, whatsappStore } = data.info;
            if (configFields.backendPath) configFields.backendPath.textContent = backendPath || 'No disponible';
            if (configFields.whatsappStore) configFields.whatsappStore.textContent = whatsappStore || 'No disponible';
            if (configFields.dbHost) configFields.dbHost.textContent = db?.host || 'No definido';
            if (configFields.dbName) configFields.dbName.textContent = db?.name || 'No definido';
            if (configFields.dbUser) configFields.dbUser.textContent = db?.user || 'No definido';
            if (configFields.dbPort) configFields.dbPort.textContent = db?.port ? String(db.port) : 'No definido';
        } catch (error) {
